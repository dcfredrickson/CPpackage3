...
void loadProfiles(struct XSFfile * XSFIN, struct radial_profile * CORE) {
    ...
    for (j = 0; j < 130; j++) {
        if (CORE -> use[j] == 1) {
            ...
            stop = 0;
            line_counter = 0;
            read_line(f3, firstline);
            integral = 0.0;
            while (stop == 0) {
                check = fscanf(f3, "%lf %lf %lf %lf", & r, & rho, & drho_dr, & d2rho_dr2);
                if ((check == 0) || (check == EOF)) stop = 1;
                else {
                    CORE -> r[j][line_counter] = r;
                    CORE -> rho[j][line_counter] = rho / (4 * PI);
                    line_counter++;
                }
            }
...
int main(int argc, char * argv[]) {
    ...
    if (argc > 1) {
        ...
        printf("Usage:  AddCoreXSF <XSF filename without extension> \n");
        exit(0);
    }
    /* CHECK THAT NEEDED FILES ARE PRESENT, AND LOAD DATA FOR DATASET 1*/
    sprintf(outfilename, "%s-full_rho.xsf", filename);
    strcat(filename, ".xsf");
    f2 = fopen(filename, "r");
    if (f2 == NULL) {
    ...
    addCores( & den, & cores);
    f4 = fopen(outfilename, "w");
    outputXSF( & den, & den, f4);
    fclose(f4);
}
