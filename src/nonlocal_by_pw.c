...
void read_calc_nonlocal(char filename[200], char outfile[200], struct wfk_file * wfkin, struct psp_file * pspin, double occ_min, int dtset) {
    ...
    for (k = 0; k < nsppol; k++) {
        for (kptno = 0; kptno < nkpt; kptno++) {
            ...
            printf(" %d bands occuppied to within %lf electrons.\n", noccband, occ_min);
            fread( & j, sizeof(int), 1, f2);
            ...
            normalization = 0.0;
            pw_counter = 0;
            pw_min = pw_step * (pw_group - 1);
            pw_max = pw_step * (pw_group);
            if (pw_max > npw) pw_max = npw;
            printf(" Analyzing interactions for planewaves %d to %d\n", pw_min, pw_max);
            for (pw1 = pw_min; pw1 < pw_max; pw1++) {
                for (atom_type = 0; atom_type < ntypat; atom_type++) {
...
int main(int argc, char * argv[]) {
    ...
    if (argc > 6) {
        ...
        sscanf(argv[5], "%d", & pw_step);
        sscanf(argv[6], "%d", & pw_group);
    } else {
        printf("Usage:  nonlocal17_by_pw <_out file> <_o base> <dtsetnum> <minimum band occupancy> <pw step> <pw group>\n");
        exit(0);
    }
    ...
    sprintf(outfilename, "%s_o_DS%d_NL-%d.log", WFKfilename, dtsetnum, pw_group);
